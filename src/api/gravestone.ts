...
export const useGetFilteredGravestones = (query: any) => {
  // console.log(query);
  const { data } = useSWR([endpoints.gravestone.getGravestones, query], fetcher);
  // console.log(data);
  const memoizedValue = {
...
export async function deleteGravestone(gravestoneId: string) {
  const data = { gravestoneId };
  const res = await axiosInstance.delete(endpoints.gravestone.delete, { data });
